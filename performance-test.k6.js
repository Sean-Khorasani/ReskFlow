...
const deliveryCreationRate = new Rate('delivery_creation_success');
...
export const options = {
  ...
  thresholds: {
    ...
    delivery_creation_success: ['rate>0.95'],        // 95% success rate
...
export default function () {
  ...
  deliveryCreationRate.add(createSuccess);
  errorRate.add(!createSuccess);

  if (createRes.status === 201) {
    const deliveryId = createRes.json('id');
    const trackingNumber = createRes.json('trackingNumber');

    sleep(2);

    // Scenario 3: Track delivery
    ...
    check(trackRes, {
      ...
      'has delivery info': (r) => r.json('delivery.id') === deliveryId,
    });
...
export function testBlockchainPerformance() {
  ...
  for (let i = 0; i < iterations; i++) {
    ...
    // Create delivery with blockchain recording
    const res = http.post(
      `${BASE_URL}/deliveries`,
