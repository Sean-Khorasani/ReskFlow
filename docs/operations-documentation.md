...
# Configuration
MONGO_URI="mongodb://delivery:password@localhost:27017/delivery"
...
pg_restore -h localhost -U postgres -d delivery_restore -v $BACKUP_FILE

# Verify restore
psql -U postgres -d delivery_restore -c "SELECT COUNT(*) FROM users;"

# Switch databases
psql -U postgres <<EOF
ALTER DATABASE delivery RENAME TO delivery_old;
ALTER DATABASE delivery_restore RENAME TO delivery;
EOF
...
class DisasterRecoveryManager:
    ...
    def promote_dr_database(self):
        ...
        response = self.rds.promote_read_replica(
            DBInstanceIdentifier='delivery-db-dr'
        )
        
        # Wait for promotion
        waiter = self.rds.get_waiter('db_instance_available')
        waiter.wait(DBInstanceIdentifier='delivery-db-dr')
