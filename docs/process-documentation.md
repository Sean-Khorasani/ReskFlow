...
runbook:
  ...
  procedure:
    ...
    4_update_configuration:
      ...
      commands:
        - kubectl set env deployment/api-gateway DATABASE_URL=postgresql://user@standby-db/delivery
        - kubectl set env deployment/order-service DATABASE_URL=postgresql://user@standby-db/delivery
...
# Database
psql -h localhost -U postgres -d delivery -c "SELECT 1;" # Test connection
pg_dump -h localhost -U postgres delivery > backup.sql   # Backup database
