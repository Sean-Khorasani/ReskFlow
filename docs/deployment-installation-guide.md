...
DATABASE_URL=postgresql://delivery:secure_password@localhost:5432/delivery
REDIS_URL=redis://localhost:6379
MONGODB_URL=mongodb://delivery:secure_password@localhost:27017/delivery
...
CREATE USER delivery WITH PASSWORD 'secure_password';
...
mongodb:
  uri: mongodb://delivery:${MONGO_PASSWORD}@localhost:27017/delivery
  options:
...
# PostgreSQL backup
PGPASSWORD=$DB_PASSWORD pg_dump -h localhost -U delivery -d delivery | gzip > $BACKUP_DIR/postgres_$DATE.sql.gz

# MongoDB backup
mongodump --uri="mongodb://delivery:$MONGO_PASSWORD@localhost:27017/delivery" --gzip --archive=$BACKUP_DIR/mongodb_$DATE.gz
...
sudo -u postgres psql -d delivery -c "VACUUM ANALYZE;"
sudo -u postgres psql -d delivery -c "REINDEX DATABASE delivery;"

# MongoDB maintenance
mongosh delivery --eval "db.runCommand({ compact: 'orders' })"
...
psql -h localhost -U delivery -d delivery -c "SELECT 1;"

# Test Redis connection
redis-cli -a $REDIS_PASSWORD ping

# Test MongoDB connection
mongosh mongodb://delivery:$MONGO_PASSWORD@localhost:27017/delivery --eval "db.runCommand({ ping: 1 })"
```
