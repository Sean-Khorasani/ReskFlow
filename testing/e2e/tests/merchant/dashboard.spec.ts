...
test.describe('Merchant Dashboard', () => {
  ...
  test('should update order status', async ({ page }) => {
    ...
    if (await confirmedOrder.isVisible()) {
      ...
      await expect(page.locator('[data-testid="order-status"]')).toContainText('Out for delivery');
    }
  });
  ...
  test('should manage restaurant settings', async ({ page }) => {
    ...
    await page.fill('[data-testid="delivery-fee"]', '4.99');
    await page.fill('[data-testid="delivery-radius"]', '5');
    
    await page.click('[data-testid="save-delivery-settings"]');
