...
test.describe('Driver Delivery Flow', () => {
  ...
  test('should display driver dashboard', async ({ page }) => {
    ...
    const currentDelivery = page.locator('[data-testid="current-delivery"]');
    if (await currentDelivery.isVisible()) {
      await expect(currentDelivery).toContainText(/Pickup|Delivery/);
    }
  });
  
  test('should go online and accept delivery', async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="delivery-card"]', { timeout: 30000 });
    
    // View first available delivery
    const firstDelivery = page.locator('[data-testid="delivery-card"]').first();
    const deliveryId = await firstDelivery.getAttribute('data-delivery-id');
    
    // Check delivery details
    await expect(firstDelivery.locator('[data-testid="pickup-location"]')).toBeVisible();
    await expect(firstDelivery.locator('[data-testid="delivery-location"]')).toBeVisible();
    await expect(firstDelivery.locator('[data-testid="estimated-distance"]')).toBeVisible();
    await expect(firstDelivery.locator('[data-testid="delivery-fee"]')).toBeVisible();
    
    // Accept delivery
    await firstDelivery.locator('[data-testid="accept-delivery"]').click();
    
    // Confirm acceptance
    await page.click('[data-testid="confirm-accept"]');
    
    // Should navigate to active delivery view
    await expect(page.locator('[data-testid="active-delivery"]')).toBeVisible();
    await expect(page.locator('[data-testid="delivery-status"]')).toContainText('Heading to pickup');
  });
  
  test('should complete delivery flow', async ({ page }) => {
    // Assuming driver has an active delivery
    const hasActiveDelivery = await page.locator('[data-testid="active-delivery"]').isVisible();
    
    if (!hasActiveDelivery) {
      // Accept a delivery first
      await page.locator('[data-testid="driver-status-toggle"]').click();
      await page.waitForSelector('[data-testid="delivery-card"]');
      await page.locator('[data-testid="accept-delivery"]').first().click();
      await page.click('[data-testid="confirm-accept"]');
    }
    ...
    await expect(page.locator('[data-testid="delivery-status"]')).toContainText('At pickup location');
    ...
    await expect(page.locator('[data-testid="delivery-status"]')).toContainText('Heading to customer');
    ...
    await expect(page.locator('[data-testid="delivery-instructions"]')).toBeVisible();
    
    // Simulate arrival at customer
    await page.click('[data-testid="arrived-at-customer"]');
    await expect(page.locator('[data-testid="delivery-status"]')).toContainText('At delivery location');
    
    // Complete delivery
    await page.click('[data-testid="complete-delivery"]');
    
    // Delivery confirmation
    await page.selectOption('[data-testid="delivery-method"]', 'handed-to-customer');
    await page.click('[data-testid="confirm-delivery"]');
    
    // Should show completion screen
    await expect(page.locator('[data-testid="delivery-completed"]')).toBeVisible();
    await expect(page.locator('[data-testid="delivery-earnings"]')).toBeVisible();
  ...
  test('should view earnings and history', async ({ page }) => {
    ...
    const firstDelivery = page.locator('[data-testid="delivery-row"]').first();
    if (await firstDelivery.isVisible()) {
      ...
      await expect(page.locator('[data-testid="delivery-route"]')).toBeVisible();
  ...
  test('should handle delivery issues', async ({ page }) => {
    // Assume driver has active delivery
    const hasActiveDelivery = await page.locator('[data-testid="active-delivery"]').isVisible();
    ...
    await page.selectOption('[data-testid="delivery-outcome"]', 'left-at-door');
