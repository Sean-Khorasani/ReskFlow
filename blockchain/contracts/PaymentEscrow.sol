    ...
    ReentrancyGuardUpgradeable
{
    ...
    function createEscrow(
        ...
        IDeliveryRegistry.Delivery memory delivery = deliveryRegistry.getDelivery(_deliveryId);
        require(delivery.id != 0, "Delivery not found");
        require(delivery.sender == msg.sender, "Not the sender");
        require(escrows[_deliveryId].amount == 0, "Escrow already exists");

        escrows[_deliveryId] = Escrow({
            amount: msg.value,
            token: address(0),
            payer: msg.sender,
        ...
        emit EscrowCreated(_deliveryId, msg.sender, msg.value, address(0));
    }
    ...
    function createTokenEscrow(
        ...
        emit EscrowCreated(_deliveryId, msg.sender, _amount, _token);
    }

    function releasePayment(bytes32 _deliveryId) external whenNotPaused nonReentrant {
        Escrow storage escrow = escrows[_deliveryId];
        require(escrow.amount > 0, "No escrow found");
        require(!escrow.released && !escrow.refunded, "Already processed");

        IDeliveryRegistry.Delivery memory delivery = deliveryRegistry.getDelivery(_deliveryId);
        require(
            delivery.status == IDeliveryRegistry.DeliveryStatus.Delivered,
            "Not delivered"
        );
        require(
            msg.sender == delivery.driver || 
        ...
        if (escrow.token == address(0)) {
            // Native token
            driverBalances[delivery.driver] += driverAmount;
            driverBalances[treasuryAddress] += platformAmount;
            totalEscrowedNative -= escrow.amount;
        } else {
            // ERC20 token
            driverTokenBalances[delivery.driver][escrow.token] += driverAmount;
            driverTokenBalances[treasuryAddress][escrow.token] += platformAmount;
            totalEscrowedTokens[escrow.token] -= escrow.amount;
        }

        emit PaymentReleased(_deliveryId, delivery.driver, driverAmount, platformAmount);
    }

    function refundPayment(bytes32 _deliveryId) external whenNotPaused nonReentrant {
        Escrow storage escrow = escrows[_deliveryId];
        require(escrow.amount > 0, "No escrow found");
        require(!escrow.released && !escrow.refunded, "Already processed");
        ...
        IDeliveryRegistry.Delivery memory delivery = deliveryRegistry.getDelivery(_deliveryId);
        require(
            delivery.status == IDeliveryRegistry.DeliveryStatus.Cancelled ||
            delivery.status == IDeliveryRegistry.DeliveryStatus.Failed,
        ...
        emit PaymentRefunded(_deliveryId, escrow.payer, escrow.amount);
    }
    ...
    // View functions
    function getEscrowDetails(bytes32 _deliveryId) external view returns (Escrow memory) {
        return escrows[_deliveryId];
    }
