...
async function main() {
  ...
  const deliveryRegistry = await upgrades.deployProxy(
    DeliveryRegistry,
    [deployer.address],
    { initializer: "initialize" }
  );
  await deliveryRegistry.waitForDeployment();
  const deliveryRegistryAddress = await deliveryRegistry.getAddress();
  console.log("DeliveryRegistry deployed to:", deliveryRegistryAddress);
  ...
  const paymentEscrow = await upgrades.deployProxy(
    ...
    [
      ...
      deliveryRegistryAddress, // delivery registry
  ...
  const gasOptimizer = await upgrades.deployProxy(
    GasOptimizer,
    [deployer.address, deliveryRegistryAddress],
    { 
      initializer: "initialize",
  ...
  const ORACLE_ROLE = await deliveryRegistry.ORACLE_ROLE();
  await deliveryRegistry.registerOracle(gasOptimizerAddress);
  ...
  for (const driver of testDrivers) {
    await deliveryRegistry.registerDriver(driver);
  ...
  const deploymentInfo = {
    ...
    contracts: {
      DeliveryRegistry: {
        address: deliveryRegistryAddress,
        deployer: deployer.address,
        deploymentBlock: (await deliveryRegistry.deploymentTransaction())?.blockNumber,
      },
      PaymentEscrow: {
  ...
  if (networkName !== "hardhat" && networkName !== "localhost") {
    ...
    try {
      await verifyContract(deliveryRegistryAddress, []);
