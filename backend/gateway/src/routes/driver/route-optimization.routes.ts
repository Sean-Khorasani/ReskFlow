...
router.post('/optimize',
  ...
  body('deliveries.*.deliveryLocation').isObject(),
  body('deliveries.*.deliveryLocation.latitude').isFloat({ min: -90, max: 90 }),
  body('deliveries.*.deliveryLocation.longitude').isFloat({ min: -180, max: 180 }),
...
router.post('/recalculate',
  ...
  body('reason').optional().isIn(['traffic', 'road_closure', 'new_delivery', 'cancelled_delivery']),
  validationMiddleware,
  async (req, res, next) => {
    try {
      const result = await proxyRequest({
