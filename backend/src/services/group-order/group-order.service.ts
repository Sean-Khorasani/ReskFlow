...
export class GroupOrderService extends EventEmitter {
  ...
  private async calculatePaymentSplits(groupOrderId: string): Promise<void> {
    ...
    const deliveryFee = groupOrder.merchant.deliveryFee || 0;
    ...
    if (groupOrder.splitEqually) {
      ...
      // Each pays for their items + proportional share of delivery/tax
      for (const participant of groupOrder.participants) {
        const itemShare = participant.subtotal;
        const deliveryShare = groupOrder.hostPaysDelivery ? 0 : 
          (deliveryFee * (participant.subtotal / subtotal));
        const taxShare = taxes * (participant.subtotal / subtotal);
        
        await prisma.groupParticipant.update({
          where: { id: participant.id },
          data: { shareAmount: itemShare + deliveryShare + taxShare },
        });
      }

      // If host pays delivery, add it to their share
    ...
    try {
      ...
      const order = await prisma.order.create({
        data: {
          ...
          deliveryFee: groupOrder.merchant.deliveryFee || 0,
