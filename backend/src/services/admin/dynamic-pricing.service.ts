...
interface PricingAction {
  ...
  target: 'delivery_fee' | 'service_fee' | 'item_price' | 'total';
  value: number;
...
export class DynamicPricingService extends EventEmitter {
    ...
    try {
      ...
      for (const order of sampleOrders) {
        const params = {
          ...
          deliveryDistance: order.deliveryDistance || 5,
        };
  ...
  async getCompetitivePricingAnalysis(
    ...
    try {
      ...
      const ourAvgPrice = ourPrices.deliveryFee + ourPrices.serviceFee;
  ...
  private calculateBasePrice(params: any): any {
    ...
    const baseDeliveryFee = this.calculateBaseDeliveryFee(params.deliveryDistance);
  ...
  private matchesScope(scope: PricingRule['scope'], params: any): boolean {
    ...
    if (scope.zones) {
      const inZone = scope.zones.some(zoneId => 
        this.isLocationInZone(params.deliveryLocation, zoneId)
      );
  ...
  private async evaluateDemandCondition(condition: PricingCondition, params: any): Promise<boolean> {
    const demandData = await this.getCurrentDemandData(params.deliveryLocation);
  ...
  private async evaluateWeatherCondition(condition: PricingCondition, params: any): Promise<boolean> {
    const weather = await this.getCurrentWeather(params.deliveryLocation);
  ...
  private async applyRule(
    ...
    for (const action of rule.actions) {
      ...
      switch (action.target) {
        case 'delivery_fee':
          newCalculation.breakdown.deliveryFee += cappedAdjustment;
          break;
        case 'service_fee':
    ...
    newCalculation.finalPrice = 
      ...
      newCalculation.breakdown.deliveryFee +
  ...
  private calculateAdjustment(action: PricingAction, calculation: PriceCalculation): number {
    ...
    switch (action.target) {
      case 'delivery_fee':
        baseAmount = calculation.breakdown.deliveryFee;
        break;
      case 'service_fee':
    ...
    return [
      {
        ...
        avgDeliveryFee: 3.99,
        avgServiceFee: 2.99,
        promotions: ['free_delivery_over_25'],
  ...
  private async getOurAveragePricing(category: string): Promise<{
    ...
    if (orders.length === 0) {
      return { deliveryFee: 3.99, serviceFee: 2.99 };
    }

    const avgDeliveryFee = orders.reduce((sum, o) => sum + (o.deliveryFee || 0), 0) / orders.length;
    const avgServiceFee = orders.reduce((sum, o) => sum + (o.serviceFee || 0), 0) / orders.length;

    return { deliveryFee: avgDeliveryFee, serviceFee: avgServiceFee };
  }
  ...
  private generateCompetitiveRecommendations(
    ...
    if (!competitorPromos.has('free_delivery')) {
      recommendations.push({
        action: 'Launch free delivery promotion to differentiate',
