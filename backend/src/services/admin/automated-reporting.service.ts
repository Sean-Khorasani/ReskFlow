  ...
  async generateReport(
    ...
    try {
      ...
      const generatedReport: GeneratedReport = {
        ...
        deliveryStatus: definition.recipients.map(r => ({
          recipientId: r.id,
  ...
  async getReportAnalytics(
    ...
    try {
      ...
      generatedReports.forEach(report => {
        report.deliveryStatus?.forEach(delivery => {
          totalDeliveries++;
          if (delivery.status === 'sent') successfulDeliveries++;
          if (delivery.status === 'failed') failedDeliveries++;
        });
      });
  ...
  private async deliverReport(
    ...
    for (const recipient of definition.recipients) {
      try {
        ...
        const deliveryStatus = generatedReport.deliveryStatus.find(
    ...
    await prisma.generatedReport.update({
      ...
      data: {
        status: 'delivered',
        deliveryStatus: generatedReport.deliveryStatus,
      },
    });
  }
