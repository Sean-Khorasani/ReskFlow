...
export class IntegrationOrchestrator extends EventEmitter {
  ...
  private async executeDriverAssignmentFlow(context: FlowContext) {
    try {
      ...
      if (!acceptedDriver) {
        throw new Error('No driver accepted the delivery');
      }
  ...
  private async findOptimalDrivers(criteria: any): Promise<any[]> {
    const response = await axios.post(getServiceUrl('delivery', 'findDrivers'), criteria);
    return response.data.drivers;
  }

  private async sendDriverRequests(drivers: any[], deliveryData: any): Promise<any> {
    const response = await axios.post(getServiceUrl('delivery', 'sendRequests'), {
      drivers,
      deliveryData,
    });
    return response.data;
  ...
  private async confirmDriverAssignment(driver: any, orderData: any): Promise<void> {
    await axios.post(getServiceUrl('delivery', 'confirm'), {
  ...
  private async enableDeliveryTracking(orderId: string, driverId: string): Promise<void> {
    await axios.post(getServiceUrl('delivery', 'enableTracking'), {
      orderId,
      driverId,
    });
  }
