...
export class SubscriptionService extends EventEmitter {
  private plans: SubscriptionPlan[] = [
    {
      id: 'reskflow_plus',
      name: 'ReskFlow Plus',
      description: 'Unlimited free delivery and exclusive perks',
      price: {
        monthly: 9.99,
        yearly: 99.99,
      },
  ...
  private async applySubscriptionBenefits(
    ...
    // Add free delivery credits if limited
  ...
  private async calculatePeriodSavings(customerId: string, periodStart: Date): Promise<number> {
    ...
    for (const order of orders) {
      // Add delivery fee savings
      if (order.deliveryFeeWaived) {
        totalSavings += order.deliveryFee;
      }
