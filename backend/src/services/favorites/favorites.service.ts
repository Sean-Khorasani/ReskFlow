  ...
  async reorder(customerId: string, orderId: string, updates?: {
    ...
    try {
      ...
      const newOrder = await orderService.createOrder({
        ...
        deliveryAddress: updates?.deliveryAddress || order.deliveryAddress,
        deliveryInstructions: updates?.deliveryInstructions || order.deliveryInstructions,
  ...
  async reorderFromSaved(customerId: string, savedOrderId: string, updates?: any): Promise<any> {
    try {
      ...
      const newOrder = await orderService.createOrder({
        ...
        deliveryAddress: updates?.deliveryAddress || customer?.addresses[0]?.formatted || '',
        deliveryInstructions: updates?.deliveryInstructions,
