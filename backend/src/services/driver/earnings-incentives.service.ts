...
export class EarningsIncentivesService extends EventEmitter {
   ...
   */
  async updateEarningsAfterDelivery(deliveryId: string): Promise<EarningsBreakdown> {
    try {
      ...
      const earnings = await this.calculateDeliveryEarnings(delivery);
      ...
      await this.updateEarningsGoals(delivery.driverId, earnings.total);

      // Check for milestone achievements
      await this.checkEarningsMilestones(delivery.driverId);

      // Send earnings notification
      await notificationService.sendDriverNotification(
        delivery.driverId,
        'Earnings Added',
        `You earned $${earnings.total.toFixed(2)} from your delivery!`,
        {
          type: 'earnings_added',
          deliveryId,
          breakdown: earnings,
   ...
   */
  private async calculateDeliveryEarnings(delivery: any): Promise<EarningsBreakdown> {
    const breakdown: EarningsBreakdown = {
      ...
      weatherBonus: 0,
      demandBonus: 0,
      tips: delivery.order.tipAmount || 0,
      incentives: 0,
      total: 0,
    };

    // Distance bonus
    const distance = delivery.actualDistance || delivery.estimatedDistance || 0;
    breakdown.distanceBonus = distance * this.PER_MILE_RATE;

    // Time bonus
    const deliveryTime = delivery.completedAt && delivery.pickedUpAt
      ? Math.floor((delivery.completedAt.getTime() - delivery.pickedUpAt.getTime()) / 60000)
      : 0;
    breakdown.timeBonus = deliveryTime * this.PER_MINUTE_RATE;

    // Get active incentives
    const incentives = await this.getApplicableIncentives(delivery);
    ...
    if (weatherIncentive && await this.checkWeatherConditions(delivery.driver.currentLocation)) {
    ...
    if (demandIncentive && await this.checkDemandLevel(delivery.order.merchantId)) {
    ...
    for (const incentive of incentives) {
      if (!['peak_hours', 'bad_weather', 'high_demand'].includes(incentive.type)) {
        const incentiveAmount = await this.calculateSpecialIncentive(delivery.driverId, incentive);
  ...
  /**
   * Get applicable incentives for delivery
   */
  private async getApplicableIncentives(delivery: any): Promise<Incentive[]> {
    ...
    return activeIncentives.filter(incentive => {
      if (incentive.applicableZones && incentive.applicableZones.length > 0) {
        // Check if delivery is in applicable zone
        return incentive.applicableZones.includes(delivery.zone || 'default');
      }
      return true;
    });
  }
  ...
  private async checkCompletionBonus(driverId: string, incentive: Incentive): Promise<number> {
    ...
    if (deliveryCount >= incentive.conditions.minDeliveries) {
  ...
  async getDriverPerformance(driverId: string, period: 'daily' | 'weekly' | 'monthly'): Promise<DriverPerformance> {
    ...
    // Get all delivery requests for acceptance rate
    const allRequests = await prisma.deliveryRequest.count({
      where: {
        driverId,
    ...
    const acceptedRequests = await prisma.deliveryRequest.count({
      where: {
        driverId,
        createdAt: { gte: startDate },
        status: 'accepted',
    ...
    for (const delivery of deliveries) {
      if (delivery.earnings) {
        performance.totalEarnings += delivery.earnings.totalAmount;
        performance.baseEarnings += delivery.earnings.baseAmount + delivery.earnings.distanceBonus + delivery.earnings.timeBonus;
        performance.tips += delivery.earnings.tips;
        performance.bonuses += delivery.earnings.peakHourBonus + delivery.earnings.weatherBonus + delivery.earnings.demandBonus;
        performance.incentives += delivery.earnings.incentives;
      }

      // Add distance
      performance.totalDistance += delivery.actualDistance || 0;

      // Calculate delivery time
      if (delivery.pickedUpAt && delivery.completedAt) {
        const deliveryTime = (delivery.completedAt.getTime() - delivery.pickedUpAt.getTime()) / 60000;
        performance.averageDeliveryTime += deliveryTime;
      }

      // Add rating
      if (delivery.rating) {
        performance.averageRating += delivery.rating.rating;
      }
    }
    ...
    const startedDeliveries = await prisma.delivery.count({
      where: {
        driverId,
        createdAt: { gte: startDate },
      },
  ...
  private getIncentiveRequirements(incentive: Incentive): string[] {
    ...
    if (incentive.conditions.streakDays) {
      requirements.push(`${incentive.conditions.streakDays}-day delivery streak`);
    }
