...
export class EmergencySOSService extends EventEmitter {
  ...
  async triggerSOS(
    ...
    try {
      ...
      // Get driver and current delivery info
      ...
      const incident: SOSIncident = {
        ...
        deliveryId: data.deliveryId || driver.currentDelivery?.id,
  ...
  async reportSafetyConcern(
    driverId: string,
    concern: {
      type: 'unsafe_pickup' | 'unsafe_delivery' | 'aggressive_customer' | 'suspicious_activity' | 'other';
      location: { latitude: number; longitude: number };
      description: string;
      orderId?: string;
  ...
  private async initiateEmergencyResponse(
    ...
    // 5. If delivery in progress, notify customer and merchant
  ...
  private async notifyDeliveryParties(incident: SOSIncident, delivery: any): Promise<void> {
    // Notify customer
    await notificationService.sendCustomerNotification(
      delivery.order.customerId,
      'Delivery Delayed',
      'Your delivery driver has encountered an issue. We are working to resolve it and will update you soon.',
      {
        type: 'delivery_emergency_delay',
    ...
    await notificationService.sendMerchantNotification(
      ...
      'Driver Emergency',
      `Driver for order #${delivery.order.orderNumber} has an emergency. Support team is handling the situation.`,
      {
