...
export class ShiftSchedulingService extends EventEmitter {
  ...
  async clockOut(driverId: string, shiftId: string, location: { lat: number; lng: number }): Promise<any> {
    try {
      ...
      const deliveryCount = shift.deliveries.filter(d => d.status === 'delivered').length;
      ...
      await notificationService.sendDriverNotification(
        driverId,
        'Shift Completed',
        `Great work! You earned $${totalEarnings.toFixed(2)} from ${deliveryCount} deliveries.`,
        {
          type: 'clock_out',
          shiftId,
          summary,
  ...
  private meetsShiftRequirements(driver: any, shift: Shift): boolean {
    // Check rating, delivery count, vehicle type, etc.
  ...
  private buildShiftTimeline(shift: any): any[] {
    ...
    // Add deliveries
    for (const delivery of shift.deliveries) {
      if (delivery.acceptedAt) {
        timeline.push({
          time: delivery.acceptedAt,
          event: `Accepted delivery to ${delivery.order.customer.name}`,
          type: 'delivery',
      ...
      if (delivery.deliveredAt) {
        timeline.push({
          ...
          event: `Completed delivery #${delivery.order.orderNumber}`,
