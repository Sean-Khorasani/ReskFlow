...
export class MetricsCollector extends EventEmitter {
  ...
  private readonly platformMetrics = {
    ...
    driverUtilization: new Gauge({
      name: 'delivery_driver_utilization',
  ...
  recordDeliveryTime(zone: string, distanceCategory: string, timeMinutes: number): void {
    this.platformMetrics.deliveryTime.observe({ zone, distance_category: distanceCategory }, timeMinutes);
  }
  ...
  private async collectRevenueMetrics(start: Date, end: Date): Promise<any> {
    ...
    return {
      ...
      deliveryFees: revenue._sum.delivery_fee || 0,
  ...
  private async collectPerformanceMetrics(start: Date, end: Date): Promise<any> {
    ...
    deliveries.forEach(delivery => {
      if (delivery.actual_delivery_time && delivery.estimated_delivery_time) {
        const actualTime = new Date(delivery.actual_delivery_time).getTime();
  ...
  private async collectSatisfactionMetrics(start: Date, end: Date): Promise<any> {
    ...
    return {
      ...
      deliveryRating: ratings._avg.delivery_rating || 0,
  ...
  private initializeServiceMetrics(): void {
    ...
    const coreServices = [
      'user', 'merchant', 'catalog', 'cart', 'payment', 'delivery',
