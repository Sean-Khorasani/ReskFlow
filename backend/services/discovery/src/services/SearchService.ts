...
interface SearchParams {
  ...
  page: number;
  limit: number;
  sortBy: 'relevance' | 'rating' | 'deliveryTime' | 'distance' | 'price';
}
...
export class SearchService {
  ...
  async searchMerchants(params: SearchParams): Promise<SearchResult> {
    ...
    if (filters.deliveryTime) {
      filter.push({
        range: { preparationTime: { lte: filters.deliveryTime } },
      });
    }

    if (filters.freeDelivery) {
      filter.push({ term: { deliveryFee: 0 } });
    }
  ...
  async getMerchantDetails(
    ...
    try {
      ...
      if (latitude && longitude && merchant.locations[0]) {
        ...
        deliveryTime = Math.round(15 + distance * 3);
      }
    ...
    return Promise.all(
      hits.map(async (hit) => {
        ...
        if (location && merchant.location) {
          ...
          merchant.deliveryTime = Math.round(15 + merchant.distance * 3);
        }
