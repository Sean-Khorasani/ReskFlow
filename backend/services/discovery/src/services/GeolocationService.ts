...
export class GeolocationService {
    ...
    try {
      ...
      const nearbyMerchants = merchants
        .map(merchant => {
          ...
          // Check if within radius and delivery zone
          if (distance > radius || distance > merchant.deliveryRadius) {
            return null;
          }
          ...
          return {
            ...merchant,
            distance,
            deliveryTime: Math.round(15 + distance * 3), // Estimate
            isCurrentlyOpen,
            primaryLocation,
          };
        })
  ...
  async checkDeliveryZone(
    ...
    try {
      ...
      return distance <= (primaryLocation.deliveryRadius || merchant.deliveryRadius);
    } catch (error) {
  ...
  async getDeliveryZones(): Promise<any[]> {
    try {
      // Get all active merchants with their delivery zones
      const merchants = await prisma.merchant.findMany({
        where: { status: MerchantStatus.ACTIVE },
        include: { locations: true },
      });
      ...
      const zones = merchants.map(merchant => {
        ...
        const radius = primaryLocation.deliveryRadius || merchant.deliveryRadius;
    ...
    try {
      ...
      for (const merchant of merchants) {
        for (const location of merchant.locations) {
          ...
          if (distance < minDistance && distance <= location.deliveryRadius!) {
