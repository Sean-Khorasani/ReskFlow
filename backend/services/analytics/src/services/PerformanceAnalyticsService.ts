...
export class PerformanceAnalyticsService {
  async getMerchantPerformance(
    ...
    const deliveryTimes = completedOrders
      .filter(o => o.delivery?.picked_up_at && o.delivery?.delivered_at)
      .map(o => o.delivery!.delivered_at!.getTime() - o.delivery!.picked_up_at!.getTime());

    const onTimeDeliveries = completedOrders.filter(o => {
      if (!o.delivery?.delivered_at || !o.estimated_delivery_time) return false;
      return o.delivery.delivered_at <= o.estimated_delivery_time;
    }).length;
    ...
    return {
      orderMetrics: {
        ...
        averageDeliveryTime: deliveryTimes.length > 0
          ? deliveryTimes.reduce((a, b) => a + b, 0) / deliveryTimes.length / 1000 / 60
          : 0,
    ...
    for (const merchant of merchants) {
      ...
      await prisma.performanceAggregation.create({
        data: {
          ...
          avg_delivery_time: metrics.orderMetrics.averageDeliveryTime,
