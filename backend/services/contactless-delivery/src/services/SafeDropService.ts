...
export class SafeDropService {
  ...
  async initiateSafeDrop(params: {
    ...
    if (!delivery || delivery.driver_id !== params.driverId) {
      throw new Error('Delivery not found or unauthorized');
    }

    // Check if safe drop is enabled
    const contactlessSettings = delivery.order.contactlessDelivery;
    ...
    const suggestedLocations = await this.getSuggestedDropLocations(params.deliveryId);
    ...
    const autoApproved = await this.checkAutoApproval(delivery.order, params.reason);
    ...
    if (autoApproved) {
      ...
      await this.notifyCustomerForApproval(safeDrop, delivery.order);
    }
  ...
  async approveSafeDrop(safeDropId: string, customerId: string): Promise<void> {
    ...
    // Notify driver
    await this.notifyDriverApproval(safeDrop.delivery_id, true);
  }
  ...
  async rejectSafeDrop(
    ...
    // Notify driver
    await this.notifyDriverApproval(safeDrop.delivery_id, false);
  }

  async getSuggestedDropLocations(deliveryId: string): Promise<SafeDropLocation[]> {
    const delivery = await prisma.delivery.findUnique({
      where: { id: deliveryId },
      include: {
        order: {
    ...
    if (buildingAmenities.hasLobby) {
      locations.push({
        ...
        distance: 0,
        available: true,
        requirements: ['Photo of delivery location', 'Lobby attendant name'],
      });
    }
    ...
    if (buildingAmenities.hasMailroom) {
      locations.push({
        ...
        distance: 0,
        available: true,
        requirements: ['Photo of delivery', 'Mailroom access'],
      });
    }
    ...
    locations.push({
      ...
      distance: 0,
      available: true,
      requirements: ['Photo of delivery location'],
    });
  ...
  private async checkAutoApproval(order: any, reason: string): Promise<boolean> {
    ...
    if (safeDropPrefs.trusted_drivers?.includes(order.delivery?.driver_id)) {
      return true;
    }
  ...
  private generateSafeDropInstructions(reason: string, location: string): string {
    const instructions: { [key: string]: string } = {
      ...
      no_safe_location: `No safe location at delivery address. Leave at ${location} and ensure it's secure.`,
  ...
  private async notifyDriverApproval(deliveryId: string, approved: boolean): Promise<void> {
    // Send notification to driver about approval status
    logger.info(`Notifying driver for delivery ${deliveryId}: ${approved ? 'approved' : 'rejected'}`);
