...
export class ContactlessDeliveryService {
  ...
  async enableContactlessDelivery(params: {
    ...
    if (!order) {
      throw new Error('Order not found or not eligible for contactless delivery');
    }
    ...
    const settings = await prisma.contactlessDelivery.upsert({
      ...
      update: {
        ...
        notify_on_delivery: params.notifyOnDelivery ?? true,
    ...
    await prisma.order.update({
      ...
      data: { 
        is_contactless: true,
        delivery_instructions: params.instructions,
      },
    });
  ...
  async updateContactlessSettings(
    ...
    const settings = await prisma.contactlessDelivery.update({
      ...
      data: {
        ...
        notify_on_delivery: updates.notifyOnDelivery,
        updated_at: new Date(),
      },
    });
    ...
    if (updates.instructions !== undefined) {
      await prisma.order.update({
        where: { id: orderId },
        data: { delivery_instructions: updates.instructions },
      });
    }
  ...
  async verifyDropoff(params: {
    ...
    if (!contactlessSettings || !contactlessSettings.enabled) {
      throw new Error('Contactless delivery not enabled for this order');
    }

    // Validate required verification
    if (contactlessSettings.require_photo && !params.photoUrl) {
      throw new Error('Photo required for delivery verification');
    }

    if (contactlessSettings.require_signature && !params.signatureData) {
      throw new Error('Signature required for delivery verification');
    }
    ...
    if (contactlessSettings.notify_on_delivery) {
  ...
  async getContactlessAnalytics(
    ...
    contactlessDeliveries.forEach(delivery => {
      const location = delivery.contactlessDelivery?.drop_location || 'unknown';
    ...
    contactlessDeliveries.forEach(delivery => {
      const verification = delivery.delivery?.verification;
      if (verification) {
    ...
    const contactlessReviews = contactlessDeliveries
      .flatMap(d => d.reviews)
      .filter(r => r.rating_type === 'delivery');
  ...
  private mapToContactlessSettings(dbSettings: any): ContactlessSettings {
    return {
      ...
      notifyOnDelivery: dbSettings.notify_on_delivery,
