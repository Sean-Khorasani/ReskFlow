...
export class PhotoUploadService {
  ...
  constructor() {
    ...
    this.bucketName = process.env.S3_BUCKET_NAME || 'delivery-photos';
  }
  ...
  async getPhotoMetadata(url: string): Promise<PhotoMetadata | null> {
    try {
      ...
      return {
        ...
        deliveryId: object.Metadata.deliveryId,
  ...
  private generateFilename(metadata: PhotoMetadata): string {
    ...
    return `${year}/${month}/${day}/${metadata.type}/${metadata.deliveryId}-${uuidv4()}.jpg`;
  }
  ...
  private async storePhotoRecord(record: UploadResult): Promise<void> {
    try {
      await prisma.deliveryPhoto.create({
        data: {
          id: uuidv4(),
          delivery_id: record.metadata.deliveryId,
          url: record.url,
