...
app.post('/api/orders/:orderId/contactless', authMiddleware, async (req, res) => {
  try {
    ...
    logger.error('Error enabling contactless delivery:', error);
    res.status(500).json({ error: 'Failed to enable contactless delivery' });
  }
});
...
// Driver routes
app.post('/api/deliveries/:deliveryId/verify-drop', authMiddleware, async (req, res) => {
  try {
...
app.post('/api/deliveries/:deliveryId/safe-drop', authMiddleware, async (req, res) => {
  try {
    const { deliveryId } = req.params;
...
app.get('/api/deliveries/:deliveryId/drop-locations', authMiddleware, async (req, res) => {
  try {
    const { deliveryId } = req.params;
    const locations = await safeDropService.getSuggestedDropLocations(deliveryId);
    res.json(locations);
  } catch (error) {
...
app.get('/health', (req, res) => {
  res.json({ status: 'ok', service: 'contactless-delivery' });
});
...
async function start() {
  try {
    ...
    app.listen(PORT, () => {
      logger.info(`Contactless delivery service running on port ${PORT}`);
    });
  } catch (error) {
