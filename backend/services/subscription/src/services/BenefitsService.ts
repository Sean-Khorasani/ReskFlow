...
export class BenefitsService {
  ...
  async initializeSubscriptionBenefits(subscriptionId: string, planBenefits: any) {
    ...
    await prisma.subscriptionBenefit.create({
      data: {
        ...
        type: 'free_delivery',
        enabled: planBenefits.free_delivery,
        metadata: {
          minimum_order: planBenefits.free_delivery_minimum,
        },
      },
    });
  ...
  async getUserBenefits(
    ...
    const result: SubscriptionBenefits = {
      freeDelivery: {
        enabled: benefitMap.get('free_delivery')?.enabled || false,
        minimumOrder: benefitMap.get('free_delivery')?.metadata?.minimum_order,
        remainingUses: benefitMap.get('free_delivery')?.remaining_uses,
      },
      reducedServiceFee: {
  ...
  async checkBenefitEligibility(
    ...
    const freeDeliveryBenefit = subscription.benefits.find(
      b => b.type === 'free_delivery' && b.enabled
    );
    if (freeDeliveryBenefit) {
      ...
      if (order.subtotal >= minimumOrder) {
        eligibleBenefits.push({
          ...
          description: `Free delivery (min order $${minimumOrder})`,
        });
      }
    }
  ...
  async resetMonthlyBenefits() {
    ...
    for (const subscription of subscriptions) {
      ...
      await prisma.subscriptionBenefit.updateMany({
        ...
        data: {
          remaining_uses: subscription.plan.benefits.free_delivery_monthly_limit,
        },
      });
  ...
  async updatePlanBenefits(subscriptionId: string, newPlanBenefits: any) {
    ...
    await prisma.subscriptionBenefit.updateMany({
      ...
      data: {
        enabled: newPlanBenefits.free_delivery,
        metadata: {
          minimum_order: newPlanBenefits.free_delivery_minimum,
        },
      },
    });
