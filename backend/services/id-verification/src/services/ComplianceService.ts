...
export class ComplianceService {
  ...
  private loadStateRequirements(): void {
    ...
    const requirements: StateRequirement[] = [
      // California
      {
        ...
        additionalRequirements: ['delivery_window_restriction', 'signature_required'],
      },
      {
        state: 'CA',
      ...
      // Texas
      {
        ...
        additionalRequirements: ['no_sunday_morning_delivery'],
      },
      // New York
      {
        state: 'NY',
        ...
        recordRetention: 365,
        reportingRequired: true,
        additionalRequirements: ['licensed_delivery_only'],
      },
  ...
  private async checkAdditionalRequirement(
    ...
    switch (requirement) {
      case 'delivery_window_restriction':
        return this.checkDeliveryWindow(order, state);
      
      case 'signature_required':
        return !!order.delivery?.signature_captured;
      
      case 'no_sunday_morning_delivery':
        const deliveryTime = dayjs(order.delivery?.delivered_at || new Date());
        return !(deliveryTime.day() === 0 && deliveryTime.hour() < 12);
      
      case 'licensed_delivery_only':
        return !!order.delivery?.driver?.license_verified;
  ...
  private async checkComplianceAdherence(verification: any): Promise<boolean> {
    ...
    const state = verification.order?.delivery_state;
  ...
  private async getOrderState(orderId: string): Promise<string> {
    ...
    return order?.delivery_state || 'CA';
  }
