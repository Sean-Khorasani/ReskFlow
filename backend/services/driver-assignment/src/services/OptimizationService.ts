...
export class OptimizationService {
    ...
    orders.forEach(order => {
      ...
      nodes.push({
        ...
        timeWindow: order.delivery_time_window ? {
          start: order.delivery_time_window.start,
          end: order.delivery_time_window.end,
        } : undefined,
    ...
    for (const driver of busyDrivers) {
      if (driver.activeDeliveries.length >= 2) {
        ...
        try {
          ...
          // Update delivery sequence if significant improvement
  ...
  private async optimizeWithConstraints(
    ...
    while (unvisited.size > 0) {
      ...
      for (const node of unvisited) {
        // Check constraints
        if (node.type === 'delivery' && !pickedUpOrders.has(node.orderId)) {
  ...
  private async calculateCurrentRouteDistance(
    ...
    for (const assignment of assignments) {
      ...
      currentLocation = assignment.deliveryLocation;
    }
  ...
  private async updateDeliverySequence(driverId: string, route: OptimizedRoute) {
    // Update delivery sequence based on optimized route
    let sequence = 1;
