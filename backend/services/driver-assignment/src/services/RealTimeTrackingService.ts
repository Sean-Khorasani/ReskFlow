...
export class RealTimeTrackingService {
  ...
  async subscribeToOrderTracking(
    ...
    if (!order || !order.delivery || !order.delivery.driver_id) {
  ...
  private async broadcastLocationUpdate(update: LocationUpdate) {
    ...
    for (const sub of subscriptions) {
      ...
      if (delivery && delivery.status !== 'delivered' && delivery.status !== 'cancelled') {
        // Calculate ETA
        const eta = await this.calculateETA(update, delivery);
  ...
  private async checkGeofenceEvents(update: LocationUpdate) {
    ...
    if (delivery.status === 'assigned') {
      ...
      if (distanceToMerchant < 100) { // Within 100 meters
        await this.handleDriverArrivedAtPickup(delivery);
      }
    }

    // Check proximity to customer (delivery)
    if (delivery.status === 'in_transit') {
      ...
      if (distanceToCustomer < 100) { // Within 100 meters
        await this.handleDriverArrivedAtDelivery(delivery);
      }
    }
  }

  private async calculateETA(update: LocationUpdate, delivery: any): Promise<Date> {
    const driverLocation = {
      latitude: update.latitude,
      longitude: update.longitude,
    };
    ...
    if (delivery.status === 'assigned' || delivery.status === 'picked_up') {
      ...
      destination = {
        latitude: delivery.pickup_location.coordinates[1],
        longitude: delivery.pickup_location.coordinates[0],
      };
      ...
      const customerLocation = {
        latitude: delivery.delivery_location.coordinates[1],
        longitude: delivery.delivery_location.coordinates[0],
      };
  ...
  private async handleDriverArrivedAtPickup(delivery: any) {
    ...
    // Notify customer
    this.io.to(`tracking:${delivery.order_id}`).emit('delivery:status', {
    ...
    // Notify merchant
    await redis.publish(`merchant:${delivery.order.merchant_id}:notifications`, JSON.stringify({
      type: 'driver_arrived',
      orderId: delivery.order_id,
  ...
  private async handleDriverArrivedAtDelivery(delivery: any) {
    // Update delivery status
    await prisma.delivery.update({
      where: { id: delivery.id },
      data: {
        status: 'at_delivery',
        arrived_at_delivery: new Date(),
      },
    });
    ...
    await redis.publish(`customer:${delivery.order.customer_id}:notifications`, JSON.stringify({
      type: 'driver_arrived',
      orderId: delivery.order_id,
