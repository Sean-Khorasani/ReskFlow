...
export class DriverPoolService {
  ...
  async getDriverPerformance(driverId: string, period: string): Promise<DriverPerformance> {
    ...
    const averageDeliveryTime = deliveryTimes.length > 0
      ? deliveryTimes.reduce((a, b) => a + b, 0) / deliveryTimes.length / 1000 / 60 // in minutes
      : 0;
    ...
    const onTimeDeliveries = deliveries.filter(d => {
      if (!d.estimated_delivery_time || !d.delivered_at) return false;
      return d.delivered_at <= d.estimated_delivery_time;
    }).length;
