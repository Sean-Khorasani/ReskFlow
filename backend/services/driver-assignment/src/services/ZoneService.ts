...
export class ZoneService {
  ...
  async initializeZones() {
    logger.info('Initializing delivery zones');
    ...
    for (const zone of dbZones) {
      this.zones.set(zone.id, {
        ...
        averageDeliveryTime: zone.average_delivery_time,
      });
    }

    logger.info(`Loaded ${this.zones.size} delivery zones`);
  ...
  async getZoneStatistics(zoneId: string): Promise<ZoneStatistics> {
    ...
    const activeOrders = await prisma.delivery.count({
      where: {
        zone_id: zoneId,
    ...
    const pendingOrders = await prisma.delivery.count({
      where: {
        zone_id: zoneId,
        status: 'pending',
      },
    ...
    const recentOrders = await prisma.delivery.findMany({
      where: {
        zone_id: zoneId,
