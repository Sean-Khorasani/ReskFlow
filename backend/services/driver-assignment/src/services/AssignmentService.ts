...
export class AssignmentService {
  ...
  async getDriverAssignments(driverId: string) {
    ...
    return assignments.map(assignment => ({
      ...
      pickupLocation: {
        latitude: assignment.delivery.order.merchant.latitude,
        longitude: assignment.delivery.order.merchant.longitude,
        address: assignment.delivery.order.merchant.address,
      },
      deliveryLocation: {
        latitude: assignment.delivery.order.delivery_address.latitude,
        longitude: assignment.delivery.order.delivery_address.longitude,
        address: assignment.delivery.order.delivery_address.formatted_address,
      },
  ...
  private async createAssignment(order: any, driver: any) {
    const delivery = await prisma.delivery.create({
      data: {
        ...
        delivery_location: {
          type: 'Point',
          coordinates: [order.delivery_address.longitude, order.delivery_address.latitude],
        },
  ...
  private async calculateDeliveryEstimates(driver: any, pickup: any, delivery: any) {
    // Calculate distances
    ...
    const toDelivery = geolib.getDistance(pickup, delivery);
    ...
    const deliveryTimeSeconds = toDelivery / avgSpeedMps + 180; // +3 min for delivery

    return {
      pickupTime: new Date(Date.now() + pickupTimeSeconds * 1000),
      deliveryTime: new Date(Date.now() + (pickupTimeSeconds + deliveryTimeSeconds) * 1000),
      totalDistance,
    };
  }
