...
export class PreferenceService {
    ...
    if (existing) {
      await prisma.userPreferences.update({
        ...
        data: {
          ...
          delivery_preferences: updates.deliveryPreferences,
          updated_at: new Date(),
        },
      });
    } else {
  ...
  async getPersonalizationScore(userId: string): Promise<{
    ...
    if (preferences) {
      ...
      if (preferences.delivery_preferences) profileCompleteness += 20;
  ...
  private async createDefaultPreferences(
    ...
    return await prisma.userPreferences.create({
      data: {
        ...
        delivery_preferences: initialData?.deliveryPreferences || {
          speed: 'standard',
          contactless: true,
        },
        created_at: new Date(),
  ...
  private mergePreferences(stored: any, learned: any): UserPreferences {
    return {
      ...
      deliveryPreferences: stored.delivery_preferences,
    };
  }
