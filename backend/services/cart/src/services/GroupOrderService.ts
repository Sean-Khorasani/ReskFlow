...
export class GroupOrderService {
  ...
  private recalculateTotals(groupOrder: GroupOrder): void {
    ...
    groupOrder.totals = {
      ...
      total: subtotal + tax + groupOrder.totals.deliveryFee + serviceFee + (groupOrder.totals.tip || 0),
    };
  }
  ...
  private calculateShares(groupOrder: GroupOrder): void {
    ...
    const sharedCosts = groupOrder.totals.deliveryFee + groupOrder.totals.serviceFee + groupOrder.totals.tax;
    const sharedCostPerPerson = sharedCosts / participantCount;

    groupOrder.participants.forEach(participant => {
      // Each person pays their items + share of delivery/service/tax
