...
export class BatchMonitoringService {
  async getBatchMetrics(batchId: string): Promise<BatchMetrics> {
    ...
    const pickupsCompleted = batch.orders.filter(
      o => o.delivery && ['picked_up', 'in_transit', 'delivered'].includes(o.delivery.status)
    ).length;

    const deliveriesCompleted = batch.orders.filter(
      o => o.delivery && o.delivery.status === 'delivered'
    ).length;

    const totalStops = batch.orders.length * 2; // Pickup + delivery for each order
    ...
    const firstPickup = batch.orders
      .filter(o => o.delivery?.picked_up_at)
      .sort((a, b) => a.delivery!.picked_up_at!.getTime() - b.delivery!.picked_up_at!.getTime())[0];

    const lastDelivery = batch.orders
      .filter(o => o.delivery?.delivered_at)
      .sort((a, b) => b.delivery!.delivered_at!.getTime() - a.delivery!.delivered_at!.getTime())[0];

    // Calculate performance
    const onTimeDeliveries = batch.orders.filter(o => {
      if (!o.delivery?.delivered_at || !o.delivery.estimated_delivery_time) return false;
      return o.delivery.delivered_at <= o.delivery.estimated_delivery_time;
    }).length;
    ...
    return {
      ...
      timing: {
        ...
        firstPickupAt: firstPickup?.delivery?.picked_up_at || undefined,
        lastDeliveryAt: lastDelivery?.delivery?.delivered_at || undefined,
  ...
  async getBatchTracking(batchId: string): Promise<BatchTracking> {
    ...
    for (const node of route.nodes) {
      ...
      if (node.type === 'pickup' && order.delivery?.picked_up_at) {
        completedStops.push({
          ...stopData,
          completedAt: order.delivery.picked_up_at,
        });
      } else if (node.type === 'delivery' && order.delivery?.delivered_at) {
        completedStops.push({
          ...stopData,
          completedAt: order.delivery.delivered_at,
        });
      } else {
        ...
        if (isCurrentStop) {
          currentStop = {
            ...stopData,
            arrivedAt: order.delivery?.arrived_at_pickup || 
                      order.delivery?.arrived_at_delivery,
          };
        } else {
          upcomingStops.push({
            ...stopData,
