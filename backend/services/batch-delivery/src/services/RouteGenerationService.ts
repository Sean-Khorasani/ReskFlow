...
export class RouteGenerationService {
  ...
  async generateOptimalRoute(
    ...
    const strategies = [
      () => this.clusterFirstRouting(pickupNodes, deliveryNodes),
      () => this.nearestNeighborRouting(pickupNodes, deliveryNodes),
      () => this.savingsAlgorithm(pickupNodes, deliveryNodes),
    ];
  ...
  private nearestNeighborRouting(
    ...
    const unvisitedDeliveries = new Set(deliveryNodes);
    ...
    while (unvisitedPickups.size > 0 || unvisitedDeliveries.size > 0) {
      ...
      // Look for nearest delivery (if order picked up)
      ...
      if (nearest) {
        ...
        // Force pickup if no valid delivery
  ...
  private savingsAlgorithm(
    ...
    // Clarke-Wright savings algorithm adapted for pickup-delivery
    const allNodes = [...pickupNodes, ...deliveryNodes];
  ...
  private optimizeDeliverySequence(
    ...
    const remaining = new Set(deliveryNodes);
  ...
  private canAddToRoute(
    ...
    // Check if delivery can be added (pickup must be done)
    if (node1.type === 'delivery') {
  ...
  private checkRouteConstraints(route: RouteNode[], orders: any[]): string[] {
    ...
    for (const node of route) {
      if (node.type === 'delivery') {
        ...
        const deliveryIndex = route.indexOf(node);
        
        if (pickupIndex === -1 || pickupIndex > deliveryIndex) {
