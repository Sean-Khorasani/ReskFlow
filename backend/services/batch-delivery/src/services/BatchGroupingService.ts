...
export class BatchGroupingService {
  ...
  async checkBatchFeasibility(orders: Order[]): Promise<BatchFeasibility> {
    ...
    const deliveryCentroid = this.calculateCentroid(
      orders.map(o => ({
    ...
    const maxDeliveryDistance = Math.max(
      ...orders.map(o =>
        geolib.getDistance(deliveryCentroid, {
          latitude: o.delivery_address.latitude,
          longitude: o.delivery_address.longitude,
        })
      )
    ...
    if (estimatedDuration > this.MAX_DELIVERY_TIME) {
      return {
        feasible: false,
        reason: 'Estimated delivery time too long',
  ...
  async splitByGeography(orders: Order[]): Promise<Order[][]> {
    ...
    // Use k-means clustering on delivery locations
    const points = orders.map(o => [
      o.delivery_address.latitude,
      o.delivery_address.longitude,
    ]);
    ...
    for (const order of orders) {
      ...
      for (const other of orders) {
        ...
        const deliveryDistance = geolib.getDistance(
          ...
          {
            latitude: other.delivery_address.latitude,
            longitude: other.delivery_address.longitude,
          }
        );

        if (
          pickupDistance < this.MAX_PICKUP_RADIUS / 2 &&
          deliveryDistance < this.MAX_DELIVERY_RADIUS / 2
        ) {
          group.push(other);
          used.add(other.id);
        }
  ...
  private groupByTimeWindow(
    ...
    orders.forEach(order => {
      const timeKey = order.delivery_time_window
        ? `${order.delivery_time_window.start.getHours()}-${order.delivery_time_window.end.getHours()}`
        : 'asap';
  ...
  private checkTimeWindowsCompatibility(orders: Order[]): boolean {
    ...
    for (const order of orders) {
      if (order.delivery_time_window) {
        const start = order.delivery_time_window.start.getTime();
        const end = order.delivery_time_window.end.getTime();
  ...
  private async calculateRouteDistances(orders: Order[]) {
    ...
    const deliveryPoints = orders.map(o => ({
      latitude: o.delivery_address.latitude,
      longitude: o.delivery_address.longitude,
    }));
  ...
  private estimateDeliveryDuration(orders: Order[], distance: number): number {
    ...
    const deliveryTime = 3 * orders.length; // 3 min per delivery

    return travelTime + pickupTime + deliveryTime;
  }

  private calculateCustomerProximityScore(orders: Order[]): number {
    // Calculate how close delivery addresses are to each other
    const deliveryPoints = orders.map(o => ({
      latitude: o.delivery_address.latitude,
      longitude: o.delivery_address.longitude,
    }));
    ...
    for (let i = 0; i < deliveryPoints.length; i++) {
      for (let j = i + 1; j < deliveryPoints.length; j++) {
        totalDistance += geolib.getDistance(deliveryPoints[i], deliveryPoints[j]);
        count++;
      }
    }
