...
export class RouteOptimizer {
  ...
  async optimizeRoute(params: {
    ...
    const { driverId, deliveryIds, startLocation, constraints = {} } = params;
  ...
  private buildOptimizedRoute(
    ...
    for (let i = 0; i < sequence.length; i++) {
      ...
      const deliveryIndex = Math.floor((nextIndex - 1) / 2);
      ...
      route.push({
        ...
        distance,
        duration,
        address: isPickup ? delivery.pickupAddress : delivery.deliveryAddress,
      });
    ...
    try {
      await prisma.$executeRaw`
        INSERT INTO route_optimizations (
          driver_id, delivery_ids, route_data, 
          total_distance, total_duration, savings, 
          created_at
        ) VALUES (
          ${result.driverId},
          ${JSON.stringify(result.deliveryIds)},
