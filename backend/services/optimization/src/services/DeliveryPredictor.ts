...
export class DeliveryPredictor {
  ...
  private async loadModel() {
    try {
      ...
      this.model = await tf.loadLayersModel('file://./models/delivery_predictor/model.json');
      
      // Load feature scaler parameters
      const scalerData = await import('./models/delivery_predictor/scaler.json');
  ...
  private async trainModel() {
    try {
      ...
      // Save model
      await this.model.save('file://./models/delivery_predictor');
  ...
  private prepareTrainingData(data: any[]): {
    ...
    data.forEach(delivery => {
      if (!delivery.actualPickup || !delivery.actualDelivery) return;

      const actualDuration = 
        (delivery.actualDelivery.getTime() - delivery.actualPickup.getTime()) / 
      ...
      const hour = delivery.actualPickup.getHours();
      const dayOfWeek = delivery.actualPickup.getDay();
      const isWeekend = dayOfWeek === 0 || dayOfWeek === 6 ? 1 : 0;
      const isRushHour = (hour >= 7 && hour <= 9) || (hour >= 17 && hour <= 19) ? 1 : 0;
