...
let deliveryPredictor: DeliveryPredictor;
...
async function startService() {
  try {
    ...
    deliveryPredictor = new DeliveryPredictor();
    ...
    app.post('/optimize-route', async (req, res) => {
      try {
        const { driverId, deliveryIds, startLocation, constraints } = req.body;
    ...
    app.post('/predict-delivery-time', async (req, res) => {
      try {
        const { origin, destination, packageDetails, timeOfDay, traffic } = req.body;
        
        const prediction = await deliveryPredictor.predictDeliveryTime({
          origin,
          destination,
          packageDetails,
          timeOfDay,
