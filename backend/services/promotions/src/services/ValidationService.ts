...
export class ValidationService {
  private promotionSchema = Joi.object({
    ...
    type: Joi.string()
      .valid('percentage', 'fixed', 'bogo', 'bundle', 'loyalty', 'free_delivery')
      .required(),
    ...
    conditions: Joi.object({
      ...
      deliveryTypes: Joi.array().items(Joi.string()).optional(),
  ...
  async validatePromotionEligibility(
    ...
    // Check delivery type
    if (conditions.deliveryTypes && orderDetails) {
      if (!conditions.deliveryTypes.includes(orderDetails.deliveryType)) {
        return false;
      }
    }
  ...
  async validateDiscountStack(
    ...
    if (types.filter(t => t === 'free_delivery').length > 1) {
      errors.push('Multiple free delivery promotions cannot be applied');
    }
