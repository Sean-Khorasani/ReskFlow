...
export class DiscountService {
  ...
  async calculateDiscounts(calculation: DiscountCalculation): Promise<DiscountResult> {
    ...
    let currentDeliveryFee = calculation.deliveryFee;
    ...
    if (calculation.couponCode) {
      ...
      if (couponDiscount) {
        ...
        } else if (couponDiscount.appliedTo === 'delivery') {
    ...
    for (const promotion of promotions) {
      ...
      if (discount && !this.conflictsWithExisting(discount, appliedDiscounts)) {
        ...
        } else if (discount.appliedTo === 'delivery') {
    ...
    const totalDiscount = calculation.subtotal - currentSubtotal + 
                         calculation.deliveryFee - currentDeliveryFee;
    ...
    try {
      ...
      await prisma.order.update({
        ...
        data: {
          ...
          total: order.subtotal + order.delivery_fee - discount.amount,
        },
      });
  ...
  private calculatePromotionDiscount(
    ...
    let appliedTo: 'order' | 'items' | 'delivery' = 'order';
    ...
    switch (promotion.type) {
      ...
      case 'free_delivery':
        appliedTo = 'delivery';
        discountAmount = promotion.value; // Will be capped by actual delivery fee
        break;
    }
