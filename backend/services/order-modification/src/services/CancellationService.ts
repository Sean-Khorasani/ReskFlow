...
export class CancellationService {
  ...
  async getCancellationPolicy(orderId: string): Promise<CancellationPolicy> {
    ...
    const timeToDelivery = order.estimated_delivery_time
      ? dayjs(order.estimated_delivery_time).diff(dayjs(), 'minute')
      : null;
    ...
    if (timeToDelivery && timeToDelivery < 30 && canCancel) {
      penaltyAmount += order.delivery_fee;
      rules.push({
        stage: 'near_delivery',
        refundPercentage: refundPercentage - 10,
        penalty: order.delivery_fee,
    ...
    return {
      ...
      refundPercentage,
      penaltyAmount,
      cutoffTime: order.estimated_delivery_time
        ? dayjs(order.estimated_delivery_time).subtract(30, 'minute').toDate()
        : undefined,
      rules,
    };
  }
  ...
  private async notifyCancellation(order: any, cancellation: any): Promise<void> {
    ...
    if (order.delivery?.driver_id) {
      await this.sendCancellationNotification(order.delivery.driver_id, {
        orderId: order.id,
        reason: cancellation.reason,
      });
    }
  ...
  private async handleDriverCompensation(order: any, cancellation: any): Promise<void> {
    ...
    if (order.delivery.status === 'assigned') {
      compensationAmount = order.delivery_fee * 0.25;
    } else if (order.delivery.status === 'arrived_at_pickup') {
      compensationAmount = order.delivery_fee * 0.5;
    } else if (order.delivery.status === 'picked_up') {
      compensationAmount = order.delivery_fee * 0.75;
    }
