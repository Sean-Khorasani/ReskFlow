...
export class ModificationValidationService {
  ...
  async canModifyOrder(order: any): Promise<ValidationResult> {
    ...
    if (order.delivery && ['en_route', 'arrived', 'delivered'].includes(order.delivery.status)) {
      return {
        allowed: false,
        reason: 'Cannot modify order after delivery has started',
      };
    }
  ...
  async canCancelOrder(order: any, initiatedBy: string): Promise<ValidationResult> {
    ...
    if (user.role === 'DRIVER' && order.delivery?.driver_id === initiatedBy) {
      return { allowed: true };
    }
  ...
  private async validateAddressChange(
    ...
    // Check if new address is within delivery zone
    ...
    if (!inZone) {
      return {
        valid: false,
        error: 'New address is outside delivery zone',
      };
    }
    ...
    const warnings: string[] = [];
    if (newDeliveryFee > order.delivery_fee) {
      warnings.push(`Delivery fee will increase by $${(newDeliveryFee - order.delivery_fee).toFixed(2)}`);
    }
  ...
  private async checkDeliveryZone(
    ...
    // This would check if the location is within merchant's delivery zone
    // For now, return true
    return true;
  }
  ...
  private async calculateDeliveryFee(
    ...
    // This would calculate delivery fee based on distance
