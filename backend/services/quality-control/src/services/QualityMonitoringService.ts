...
interface QualityAlert {
  id: string;
  merchantId: string;
  type: 'accuracy' | 'delivery' | 'quality' | 'satisfaction';
...
export class QualityMonitoringService {
  ...
  async getQualityMetrics(
    ...
    return {
      ...
      onTimeDelivery: deliveryData.score,
  ...
  async getBenchmarks(
    ...
    const benchmarks: QualityBenchmark[] = [
      ...
      },
      {
        ...
        topPerformer: 0.95,
        percentile: this.calculatePercentile(merchantMetrics.onTimeDelivery, 'delivery'),
      },
      {
  ...
  private async calculateOnTimeDelivery(
    ...
    const result = await prisma.$queryRaw<any>`
      SELECT 
        ...
        COUNT(CASE WHEN 
          delivered_at <= estimated_delivery_time + INTERVAL '10 minutes' 
          THEN 1 END) as on_time_deliveries,
        AVG(EXTRACT(EPOCH FROM (delivered_at - estimated_delivery_time))/60) as avg_delay_minutes
      FROM orders
  ...
  private generateRecommendations(data: any): string[] {
    ...
    if (data.delivery.score < 0.90) {
      recommendations.push('Review delivery time estimates and adjust if needed');
      if (data.delivery.details.averageDelay > 15) {
        recommendations.push('Consider hiring additional delivery drivers during peak hours');
      }
    }
  ...
  private async generateQualityAlerts(
    ...
    const deliverySeverity = this.getSeverity(metrics.onTimeDelivery, 'onTime');
    if (deliverySeverity) {
      alerts.push({
        merchantId,
        type: 'delivery',
        ...
        metric: 'On-Time Delivery',
        threshold: this.THRESHOLDS.onTime[deliverySeverity],
        actualValue: metrics.onTimeDelivery,
        description: `On-time delivery rate (${(metrics.onTimeDelivery * 100).toFixed(1)}%) is below ${deliverySeverity} threshold`,
      });
    }
