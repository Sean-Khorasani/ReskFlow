...
interface FeedbackQuestion {
  ...
  required: boolean;
  options?: string[];
  category: 'overall' | 'food' | 'delivery' | 'packaging' | 'service';
}
...
export class FeedbackCollectionService extends EventEmitter {
  ...
  private generateDynamicQuestions(order: any): FeedbackQuestion[] {
    ...
    if (order.delivery_time > order.estimated_delivery_time) {
      questions.push({
        id: 'late-delivery-reason',
        type: 'multiChoice',
      ...
      if (category.score < 3) {
        switch (category.category) {
          case 'food':
            ...
            break;
          case 'delivery':
            actions.push('Review delivery driver performance');
            actions.push('Optimize delivery routes');
            break;
          case 'packaging':
