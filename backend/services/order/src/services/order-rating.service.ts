...
export class OrderRatingService {
  async createRating(data: CreateRatingDto): Promise<OrderRating> {
    ...
    // For pickup/dine-in orders, delivery rating is not required
    if (order.deliveryType !== 'DELIVERY' && data.deliveryRating) {
      data.deliveryRating = undefined;
    }
  ...
  async getMerchantRatings(
    ...
    return {
      ...
      averages: {
        ...
        delivery: averages._avg.deliveryRating || 0,
