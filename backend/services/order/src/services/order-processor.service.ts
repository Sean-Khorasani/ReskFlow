...
export async function startOrderProcessor() {
  ...
  await consumeMessage(QUEUES.DELIVERY_UPDATED, async (message) => {
    try {
      logger.info('Processing delivery update:', message);
      
      const { orderId, status, deliveryId } = message;
      ...
      if (orderStatus) {
        ...
        // Update delivery ID if provided
        if (deliveryId) {
          await prisma.order.update({
            where: { id: orderId },
            data: { deliveryId },
      ...
      logger.error('Failed to process delivery update:', error);
      throw error;
    }
  });
  ...
  await consumeMessage(QUEUES.CART_CHECKOUT, async (message) => {
    try {
      ...
      const { cartId, userId, deliveryType, deliveryAddress, paymentMethodId } = message;
