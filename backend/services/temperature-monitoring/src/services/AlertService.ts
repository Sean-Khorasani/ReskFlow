...
export class AlertService {
    ...
    const rules: AlertRule[] = [
      {
        ...
        severity: 'critical',
        actions: ['notify_all', 'stop_delivery'],
        enabled: true,
      },
      {
  ...
  private async notifyRecipient(alert: any, recipient: AlertRecipient): Promise<void> {
    ...
    switch (recipient.role) {
      case 'driver':
        userId = alert.order?.delivery?.driver_id;
        break;
      case 'merchant':
  ...
  private async executeAlertActions(alert: any, actions: string[]): Promise<void> {
    for (const action of actions) {
      switch (action) {
        case 'stop_delivery':
          if (alert.order?.delivery) {
            await this.stopDelivery(alert.order.delivery.id);
          }
          break;
        case 'notify_all':
  ...
  private async stopDelivery(deliveryId: string): Promise<void> {
    ...
    logger.warn(`Delivery ${deliveryId} put on hold due to temperature violation`);
  }
