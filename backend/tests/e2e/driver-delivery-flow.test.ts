 ...
 * Tests the complete flow for drivers from login to delivery completion
 */
...
test.describe('Driver Delivery Flow', () => {
  ...
  test('should go online and receive delivery requests', async ({ page }) => {
    // Set auth token
    ...
    await page.waitForSelector('[data-testid="delivery-request"]', { timeout: 60000 });
    
    // Verify request details
    await expect(page.locator('[data-testid="pickup-location"]')).toBeVisible();
    await expect(page.locator('[data-testid="delivery-location"]')).toBeVisible();
    await expect(page.locator('[data-testid="estimated-earnings"]')).toBeVisible();
  });

  test('should accept delivery request', async ({ page }) => {
    // Accept the delivery
    await page.click('[data-testid="accept-delivery-btn"]');
    
    // Verify navigation to delivery details
    await page.waitForURL('**/delivery/*');
    
    // Verify delivery status
    await expect(page.locator('[data-testid="delivery-status"]')).toContainText('Heading to Restaurant');
  ...
  test('should navigate to restaurant and confirm arrival', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="delivery-status"]')).toContainText('At Restaurant');
  ...
  test('should navigate to customer and complete delivery', async ({ page }) => {
    // Simulate arrival at customer location
    await page.click('[data-testid="arrived-at-customer-btn"]');
    
    // Verify status update
    await expect(page.locator('[data-testid="delivery-status"]')).toContainText('At Customer Location');
    ...
    await page.click('[data-testid="delivery-method-handed"]');
    ...
    if (photoRequired) {
      ...
      await page.setInputFiles('[data-testid="photo-input"]', 'tests/fixtures/delivery-proof.jpg');
    }
    
    // Submit completion
    await page.click('[data-testid="submit-delivery-btn"]');
    
    // Verify completion
    await expect(page.locator('[data-testid="delivery-complete-message"]')).toBeVisible();
  ...
  test('should handle multiple delivery requests', async ({ page }) => {
    // Go online
    await page.click('[data-testid="online-toggle"]');
    
    // Wait for batch delivery request
    await page.waitForSelector('[data-testid="batch-delivery-request"]', { timeout: 60000 });
