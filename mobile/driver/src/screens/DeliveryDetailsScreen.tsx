...
export default function DeliveryDetailsScreen() {
  ...
  const getNextStatus = () => {
    switch (order?.status) {
      ...
      case 'picked_up':
        return { status: 'on_the_way', label: 'Start Delivery', icon: 'truck-delivery' };
      case 'on_the_way':
  ...
  const region = {
    ...
    latitudeDelta: Math.abs(order.pickupLocation.latitude - order.deliveryLocation.latitude) * 1.5,
    longitudeDelta: Math.abs(order.pickupLocation.longitude - order.deliveryLocation.longitude) * 1.5,
  };
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        <Polyline
          coordinates={[order.pickupLocation, order.deliveryLocation]}
        ...
        {(order.status === 'picked_up' || order.status === 'on_the_way') && (
          <View style={styles.locationCard}>
            ...
            <View style={styles.actionButtons}>
              ...
              <TouchableOpacity
                style={styles.actionButton}
                onPress={() => handleNavigation(order.deliveryLocation, order.customerName)}
              >
                <Icon name="navigation" size={20} color={colors.primary} />
                <Text style={styles.actionButtonText}>Navigate</Text>
              </TouchableOpacity>
