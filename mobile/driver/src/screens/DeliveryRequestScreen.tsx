...
export default function DeliveryRequestScreen() {
  ...
  useEffect(() => {
    ...
    if (socket) {
      socket.on('delivery-request', (data: DeliveryRequest) => {
        setRequest(data);
        setTimeLeft(30);
        // Animate entry
    ...
    return (
      <View style={styles.container}>
        <View style={styles.waitingContainer}>
          <Icon name="truck-delivery" size={100} color={colors.gray400} />
          <Text style={styles.waitingTitle}>Waiting for delivery requests...</Text>
  ...
  const region = {
    latitude: (request.pickupLocation.latitude + request.deliveryLocation.latitude) / 2,
    longitude: (request.pickupLocation.longitude + request.deliveryLocation.longitude) / 2,
    latitudeDelta: Math.abs(request.pickupLocation.latitude - request.deliveryLocation.latitude) * 1.5,
    longitudeDelta: Math.abs(request.pickupLocation.longitude - request.deliveryLocation.longitude) * 1.5,
  };
  ...
  return (
    <Animated.View style={[styles.container, { opacity: fadeAnim }]}>
      ...
      >
        ...
        <Marker
          coordinate={request.deliveryLocation}
          title="Delivery"
        ...
        <Polyline
          coordinates={[request.pickupLocation, request.deliveryLocation]}
