...
export const HomeScreen: React.FC = () => {
  ...
  const handleDeliveryAccept = async (deliveryId: string) => {
    try {
          ...
          >
            ...
            {activeDeliveries?.map((delivery: any) => (
              <Marker
                key={delivery.id}
                coordinate={{
                  latitude: delivery.nextStop.latitude,
                  longitude: delivery.nextStop.longitude,
                }}
                title={delivery.trackingNumber}
                description={delivery.nextStop.address}
              >
        ...
        {activeDeliveries?.length === 0 ? (
          ...
          activeDeliveries?.map((delivery: any) => (
            <DeliveryCard
              ...
              onAccept={() => handleDeliveryAccept(delivery.id)}
            />
          ))
        )}
      </ScrollView>
